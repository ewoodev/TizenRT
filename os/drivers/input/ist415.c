 ...
 ****************************************************************************/
static void ist415_open(struct touchscreen_s *dev);
static void ist415_close(struct touchscreen_s *dev);
...
struct touchscreen_ops_s g_ist415_ops = {
	.touch_enable = ist415_open,
	.touch_disable = ist415_close,
...
static void touch_notify(struct ist415_dev_s *dev)
{
...
static void touch_notify(struct ist415_dev_s *priv)
{
...
static void ist415_input_report_touch(struct ist415_dev_s *dev, struct touch_point_s point)
{
	if (dev->front == dev->end)	{
		touchdbg("Warning input buffer is full\n");
		return;	/* buffer full */
	}

	memcpy(&dev->vals[dev->end], &point, sizeof(struct touch_point_s));
	dev->end++;
	if (dev->end >= IST415_MAX_TOUCH_DATA) {
		dev->end = 0;
	}
	touch_notify(dev);
}
...
static int ist415_get_touch_data(struct ist415_dev_s *dev, struct touch_sample_s *buf)
{
	buf->npoints = 0;
	for (int i = 0; i < TOUCH_MAX_POINTS; i++) {
		if (dev->front == dev->end - 1 )	{
			break;
		}

		dev->front++;
		if (dev->front >= IST415_MAX_TOUCH_DATA) {
			dev->front = 0;
		}
	
		memcpy(&buf->point[buf->npoints], &dev->vals[dev->front], sizeof(struct touch_point_s));
		buf->npoints++;
	}
...
static int ist415_read(struct touchscreen_s *dev, FAR char *buffer)
{
	...
	if (priv->int_pending) {
		...
		return ist415_get_touch_data(priv, (struct touch_sample_s *)buffer);
	} else {
...
static void ist415_enable(struct ist415_dev_s *dev)
{
	if (dev->enable == true) {
		return;
	}
...
static void ist415_disable(struct ist415_dev_s *dev)
{
	if (dev->enable == false) {
		return;
	}
...
static void ist415_reset(struct ist415_dev_s *dev, bool download)
{
	touchvdbg("%s\n", __func__);
	...
	for (int i = 0; i < (left_e + 1); i++) {
		...
		if (eid == EID_COORD) {
			...
			if (p_evt_coord->tsta != TOUCH_STA_NONE) {
				...
				point.h = p_evt_coord->major;
				point.w = p_evt_coord->minor;
				point.pressure = p_evt_coord->z;
				...
				ist415_input_report_touch(dev, point);
			} else {
...
static void ist415_close(struct touchscreen_s *priv)
{
	struct ist415_dev_s *dev = priv->priv;
...
static void ist415_open(struct touchscreen_s *priv)
{
	struct ist415_dev_s *dev = priv->priv;
	if (dev->knockknock) {
...
static void ist415_autocalibration(struct ist415_dev_s *dev)
{
...
static void ist415_display_rawdata(struct ist415_dev_s *dev)
{
	uint32_t *bufslf;
	uint32_t *bufmtl;
	int tx, rx;
...
static void ist415_display_cpc(struct ist415_dev_s *dev)
{
	uint32_t *bufslf;
	uint32_t *bufmtl;
	int tx, rx;
...
static int ist415_irq_thread(int argc, char **argv)
{
	...
	struct ist415_dev_s *dev = (struct ist415_dev_s *)strtoul(argv[1], NULL, 16);
...
int ist415_initialize(const char *path, struct ist415_dev_s *dev)
{
	...
	dev->vals = (struct touch_point_s *)kmm_zalloc(sizeof(struct touch_point_s) * IST415_MAX_TOUCH_DATA);
	dev->front = 0;
	dev->end = 1;

	parm[0] = itoa((int)dev, &parm_str, 16);
	parm[1] = NULL;
	dev->pid = kernel_thread("ist415_isr", 100, 2048, (main_t)ist415_irq_thread, (FAR char *const *)parm);
