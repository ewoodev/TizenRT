...
static int ist415_read(struct touchscreen_s *upper, FAR char *buffer);
...
static void ist415_forced_release(struct ist415_dev_s *dev)
{
	...
	memset(&point, 0, sizeof(struct touch_point_s));
	...
	for (int i = 0; i < TOUCH_MAX_POINTS; i++) {
		if (dev->touched[i] == true) {
			...
			touchvdbg("Forced touch release: %d\n", i);
			dev->touched[i] = false;
		}
	}
}
		...
		if (eid == EID_COORD) {
			...
			if (p_evt_coord->tsta != TOUCH_STA_NONE) {
				...
				case TOUCH_STA_PRESS:
					if (dev->touched[point.id] == false) {
						dev->touched[point.id] = true;
					}
				...
				case TOUCH_STA_RELEASE:
					if (dev->touched[point.id]) {
						dev->touched[point.id] = false;
					}
					point.flags = TOUCH_UP;
					if (dev->log) {
...
static void ist415_lookup_work(struct ist415_dev_s *dev)
{
	...
	if (dev->irq_working == false) {
		if (dev->event_mode == true) {
			...
			if (ret) {
				touchdbg("Fail to read touch status\n");
				goto retry_handler;
			}

			if ((touch_status & TOUCH_STATUS_MASK) != TOUCH_STATUS_MAGIC) {
				touchdbg("Touch status is not corrected (0x%08X)\n", touch_status);
				goto retry_handler;
			}
			...
			if (dev->scan_count == scan_count) {
				touchdbg("Touch IC is not responded (0x%08X)\n", touch_status);
				goto retry_handler;
			}
		}
	}
	...
	(void)wd_start(dev->wdog, MSEC2TICK(5000), (wdentry_t)ist415_timer_handler, 1, (uint32_t)dev);

	return;

retry_handler:
	dev->alive_retry++;
	touchdbg("Retry touch status(%d)\n", dev->alive_retry);
	if (dev->alive_retry == IST415_MAX_ALIVE_CNT) {
		...
		(void)wd_start(dev->wdog, MSEC2TICK(500), (wdentry_t)ist415_timer_handler, 1, (uint32_t)dev);
	}
}
...
static void ist415_timer_handler(int argc, uint32_t arg1)
{
	...
	dev->event |= IST415_EVENT_LOOKUP;
	sem_post(&dev->wait_event);
}
...
static int ist415_event_thread(int argc, char **argv)
{
	...
	while (1) {
		sem_wait(&dev->wait_event);
		if (dev->event & IST415_EVENT_LOOKUP) {
			ist415_lookup_work(dev);
			dev->event &= ~IST415_EVENT_LOOKUP;

		} else if (dev->event & IST415_EVENT_IRQ) {
...
int ist415_initialize(const char *path, struct i2c_dev_s *i2c, struct ist415_config_s *lower)
{
	...
	if (!dev) {
		touchdbg("Fail to alloc ist415_dev_s\n");
		return ERROR;
	}
	dev->i2c = i2c;
	...
	if (dev->pid < 0) {
		touchdbg("Fail to create kernel thread\n");
		kmm_free(dev);
		return ERROR;
	}
	...
	dev->wdog = wd_create();
	if (dev->wdog) {
		if (wd_start(dev->wdog, MSEC2TICK(5000), (wdentry_t)ist415_timer_handler, 1, (uint32_t)dev) != OK) {
	...
	touchdbg("Touch Driver registered Successfully\n");
